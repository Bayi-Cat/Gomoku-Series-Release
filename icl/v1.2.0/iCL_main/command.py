...
while(quit != 0):  #循环,直到输入“quit”/Loop until you type "quit"
    ...
    if(typing == "help"): #输入实现/Input implementation
        ...
        user_name = input("")#输入用户名/Enter username
        if(user_name == ""):
        ...
        try:#防止输入不存在账户/Prevents entering accounts that do not exist
            ...
            if(user_password+"\n" == contents):
                ...
                user_namel = linecache.getline(f"{data_name}.txt",1)#基本信息覆盖/Basic information overlay
                ...
                uachievement = linecache.getline(f"{data_name}.txt",11)
                count_achievement = 0
